	...
	public class Kml22 extends XmlParser
	{
		...
		*/	
		public function Kml22(xmlStr:String)
		{
			...
			if (ParsingTools.nullCheck(this.x.kml::NetworkLink)) {
				this._feature = new NetworkLink(this.x.kml::NetworkLink);
			}
		...
		*/	
		...
		public function get icons()
		{
			default xml namespace = kml;
			var list:XMLList = this.x..kml::Icon;
			var h:XML;
			var href:String;
			
			var hrefs:Array = new Array();
		
			
			for each (h in list)
			{
				href = h.href;
				var index:int = href.search("http");
			
				if ((index >= 0) && (href.length > 0))
				{
					hrefs.push(href);
				}
			}
